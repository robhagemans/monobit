...
def _load_yaff(text_stream, allow_empty):
    ...
    fonts = []
    if text_stream.peek(3).startswith('---'):
        text_stream.readline()
    while True:
        font = load_one_yaff(test_stream)
...
def _load_one_yaff(text_stream, *):
    ...
    blocktypes = (YaffComment, YaffGlyph, YaffProperties, Empty, Unparsed)
    glyphs = []
    font_comments = []
    font_props = {}
    current_comment = []
    for block in iter_blocks(text_stream, blocktypes):
        ...
        elif isinstance(block, YaffGlyph):
            glyphs.append(block.get_value().modify(
                comment='\n\n'.join(current_comment),
            ))
            current_comment = []
        elif isinstance(block, YaffProperty):
            # deal with glyphs with unmarked labels
            if set(block.get_value()) == set('.@'):
                glyphblock = YaffGlyph()
                glyphblock.lines = block.lines
                glyphs.append(glyphblock.get_value().modify(
                    comment='\n\n'.join(current_comment),
                ))
                current_comment = []
            else:
                font_props[block.get_key()] == block.get_value()
...
class YaffGlyph(DrawGlyph):
    paper = '.'
    ink = '@'

    def starts(self, line):
        return line and (line[:1] in '"' + "'" + ascii.digits) or '+' in line
