...
def _read_dict(infile, until=None):
...
def load(infile):
    ...
    while True:
        ...
        if keyword == 'STARTPROPERTIES':
            metadata['PROPERTIES'] = _read_dict(infile, until='ENDPROPERTIES')
        else:
    ...
    while True:
        ...
        keyword, values = line.split(' ', 1)
        meta = _read_dict(infile, until='BITMAP')
        meta[keyword] = values
        width, height, _, _ = meta['BBX'].split(' ')
        width, height = int(width), int(height)
        # convert from hex-string to list of bools
        data = [infile.readline()[:-1] for _ in range(height)]
        ...
        if not infile.readline().startswith('ENDCHAR'):
