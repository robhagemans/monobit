...
def load_zapredraw(instream):
    ...
    if header.null_offset:
        # we should already be at the point indicated by null_offset
    ...
    for entry in chunk_dir:
        ...
        #
        # we should already be at the point indicated by offset
        ...
        part_font = load_bitmap(
            instream,
            width=header.width, height=header.height, count=entry.count,
            msb='right', align='right', byte_swap=ceildiv(header.width, 8),
        )
        glyphs.extend(
...
def save_zapredraw(fonts, outstream, raw:bool=False):
    ...
    for chunk, codepoints, translation_table in chunk_map[1:]:
        ...
        offset += len(translation_table) + len(codepoints) * glyph_size
