...
""" * 2)
    ...
    def test_render_bdf_vertical(self):
        ...
        text2 = monobit.render_text(vert2, b'\x27\x27', direction='top-to-bottom').as_text(inklevels='.@', border='.')
        assert text2 == self.verttext, f'"""{text2}"""\n != \n"""{self.verttext}"""'

    def test_render_yaff_vertical(self):
        vert1, *_ = monobit.load(self.font_path / 'vertical.yaff')
        text1 = monobit.render_text(vert1, b'\x27\x27', direction='top-to-bottom').as_text(inklevels='.@', border='.')
    ...
    def test_render_sfnt_vertical(self):
        ...
        text1 = monobit.render_text(vert1, '\x27\x27', direction='top-to-bottom').as_text(inklevels='.@', border='.')
        assert text1 == self.verttext, f'"""{text1}"""\n != \n"""{self.verttext}"""'
...
"""
    ...
    def _render_proportional(
        ...
        if char:
            ...
            text = b'testing'
        rendered_text = monobit.render_text(prop2, text).as_text(inklevels=inklevels, border=inklevels[0])
...
"""
    ...
    def _render_kerning(self, format, char=False):
        ...
        if char:
            ...
            text = b'sjifjij'
        rendered_text = monobit.render_text(webby_mod2, text).as_text(inklevels='.@', border='.')
...
"""
    ...
    def test_render_yaff_kerning_bearings(self):
        ...
        text = monobit.render_text(font, b'01234').as_text(inklevels='.@', border='.')
...
"""
    ...
    def _render_bearings(self, format, char=False, inklevels='.@', **save_kwargs):
        ...
        if char:
            text = monobit.render_text(font, '012').as_text(inklevels=inklevels, border=inklevels[0])
        else:
            text = monobit.render_text(font, b'012').as_text(inklevels=inklevels, border=inklevels[0])
...
"""
    ...
    def _render_vert_bearings(self, format, **save_kwargs):
        ...
        monobit.save(font, self.temp_path / f'positioning.{format}', format=format, **save_kwargs)
        font, *_ = monobit.load(self.temp_path / f'positioning.{format}', format=format)
        text = monobit.render_text(font, '678', direction='ttb').as_text(inklevels='.@', border='.')
